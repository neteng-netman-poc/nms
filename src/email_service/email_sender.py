...
class EmailSender:
    ...
    def send_email(self, reason: str, email: str, body: str):
        ...
        # send email to self
        reciept_email = self.message_prep(
            subject=subject,
            email=self.receipt_email_id,
        ...
        contact_person_email = self.message_prep(
            subject=os.getenv("contact_subject"),
            email=email,
            body=os.getenv("contact_body"),
        )
    ...
    def send_to_recipient(self, subject: str, content: str):
        ...
        if not self.email_id or not self.apppassword or not self.receipt_email_id:
            raise ValueError(
                "Missing email configuration (email, apppassword, or receipt_email)."
            )

        msg = self.message_prep(
            subject=subject, email=self.receipt_email_id, body=content
        )
