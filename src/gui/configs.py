...
def pull_backup_files():
    ...
    return Div(
        Div(
            Div(Ul(*[Li(Button(f, hx_post="/load_conf", hx_target="#conf", hx_swap="innerHTML", hx_vals={"file": f})) for f in files]), cls="col"),
            Div(edit_config("Choose Config", ""), cls="col", id="conf"),
            cls="row",
        ),
        cls="container",
    )
